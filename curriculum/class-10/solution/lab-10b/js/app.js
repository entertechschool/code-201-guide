...
// State object holds the holds the current state of the application (all existing CookieShops)
const state = {
  shops: [],
};
...
function renderNew(name, minimum, maximum, average) {
  let newShop = new CookieShop(name, minimum, maximum, average);
...
function renderTotals() {
  ...
  for (let hourIndex = 0; hourIndex < hours.length; hourIndex++) {
    let hourlyTotal = 0;
    for (let shopIndex = 0; shopIndex < state.shops.length; shopIndex++) {
      hourlyTotal += state.shops[shopIndex].hourlyCookies[hourIndex];
    }
...
document.getElementById('new-store').addEventListener('submit', function (event) {
  ...
  let i = 0;
  for (i; i < state.shops.length; i++) {
    if (state.shops[i].id === store.replace(' ', '').toLowerCase()) {
      exists = true;
      break;
    }
  }
  ...
  if (exists === true) {
    renderUpdate(state.shops[i], min, max, avg);
  } else {
...
function setup() {
  renderTable();
  let seattle = new CookieShop('Seattle', 23, 65, 6.3, 'seattle');
  let tokyo = new CookieShop('Tokyo', 3, 24, 1.2, 'seatac');
  let dubai = new CookieShop('Dubai', 11, 38, 3.7, 'dubai');
  let paris = new CookieShop('Paris', 20, 38, 2.3, 'paris');
  let lima = new CookieShop('Lima', 2, 16, 4.6, 'lima');
  state.shops.push(seattle, tokyo, dubai, paris, lima);
  renderAllStores();
}
